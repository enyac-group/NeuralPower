...
#                    proc = subprocess.Popen("nvidia-smi -i 1", stdout=subprocess.PIPE, shell=True)
                    ...
                    proc = subprocess.Popen(["nvidia-smi", "--query-gpu=power.draw,utilization.gpu,utilization.memory", \
                                             "--format=csv", "-lms", "1", "-i", "1"], stdout=subprocess.PIPE)
                    ...
                    for line in proc.stdout:
                        if "power" not in line:
                            tmp = line.split()
                            power_all.append(float(tmp[0]))
                            util_all.append(float(tmp[2]))
                            util_mem_all.append(float(tmp[4]))


                mean_time = np.mean(durations)
                max_power = np.max(power_all)
                mean_power_smi = np.mean(power_all)
                mean_util = np.mean(util_all)
                mean_util_mem = np.mean(util_mem_all)
#                print('Runtime: %.3f Power: %.3f' % (mean_time,mean_power_smi))
                print('Runtime: %.3f Power: %.3f Utilization Power: %.3f Utilization Memory: %.3f' \
                      % (mean_time, mean_power_smi, mean_util, mean_util_mem))
